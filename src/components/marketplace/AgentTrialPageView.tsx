...
export const AgentTrialPageView: React.FC<AgentTrialPageViewProps> = ({ agent }) => {
  ...
  useEffect(() => {
    setMessages([
      {
        role: 'assistant',
        text: `您好！我是【${agent.name}】在线试用助理。${agent.inputExample ? `您可以输入：\n"${agent.inputExample}"` : '请问有什么可以帮助您的？'}`,
      ...
      <header className="bg-white border-b border-slate-200/80 px-6 py-4 flex items-center justify-between shadow-xs shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-xl text-slate-500 hover:text-slate-800 hover:bg-slate-100 transition border border-slate-100 bg-slate-50"
            title="清空并重置对话"
          >
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition bg-slate-50 border border-slate-100"
            title="关闭当前页面"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <aside className="w-80 bg-white border-r border-slate-200/80 p-6 flex flex-col justify-between overflow-y-auto shrink-0 hidden md:flex">
          <div className="space-y-6">
            <div className="space-y-2">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider">试用智能体详情</h3>
            ...
            <div className="space-y-3.5">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider">今日试用额度配额</h3>
          ...
          <div className="bg-white border-t border-slate-200/80 p-4 shrink-0">
            <div className="max-w-4xl mx-auto flex items-center gap-3">
              <input
                ...
                placeholder={trialCountLeft <= 0 && !userSubscription && !isPayPerTokenMode ? "试用次数已达上限" : `向 ${agent.name} 输入指令或提问（剩余试用：${trialCountLeft}次）...`}
